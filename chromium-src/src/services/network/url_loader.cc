...
URLLoader::URLLoader(
  ...
  // Initialize DataSipper network observer with Mojo client from holder
  datasipper_observer_ = std::make_unique<DataSipperNetworkObserver>(
      DataSipperMojoClientHolder::GetInstance()->GetClient());
...
void URLLoader::ScheduleStart() {
  ...
  if (defer) {
    ...
    // Notify DataSipper observer that request is starting
    if (datasipper_observer_) {
      datasipper_observer_->OnRequestStarted(url_request_.get());
    }
    url_request_->Start();
  }
}
...
void URLLoader::OnResponseStarted(net::URLRequest* url_request, int net_error) {
  ...
  if (datasipper_observer_) {
    datasipper_observer_->OnResponseStarted(url_request, response_.get());
  }
...
void URLLoader::OnReadCompleted(net::URLRequest* url_request, int bytes_read) {
  ...
  // Notify DataSipper observer about completed read
  if (datasipper_observer_ && bytes_read > 0) {
...
void URLLoader::NotifyCompleted(int error_code) {
  // Notify DataSipper observer about response completion
  if (datasipper_observer_) {
