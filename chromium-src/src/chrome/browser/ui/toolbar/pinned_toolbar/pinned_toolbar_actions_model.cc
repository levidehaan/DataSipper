...
void PinnedToolbarActionsModel::MaybeMigrateExistingPinnedStates() {
  ...
  if (!pref_service_->GetBoolean(prefs::kDataSipperMigrationComplete)) {
    UpdatePinnedState(kActionSidePanelShowDataSipper, true);
    pref_service_->SetBoolean(prefs::kDataSipperMigrationComplete, true);
  }
}
