...
// DataSipper side panel is registered via WebUIConfig in
...
WebUIFactoryFunction GetWebUIFactoryFunction(WebUI* web_ui,
                                             Profile* profile,
                                             const GURL& url) {
  // Check for chrome:// scheme URLs first
  if (content::HasWebUIScheme(url)) {
    // No direct mapping needed here for Top-Chrome WebUIs. They are created
    // via WebUIConfig when navigated to by SidePanel or TopChrome components.
  }

  // Then check for DevTools
#if BUILDFLAG(ENABLE_DEVTOOLS_FRONTEND)
