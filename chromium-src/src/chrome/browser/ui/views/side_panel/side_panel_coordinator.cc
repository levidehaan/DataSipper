...
void SidePanelCoordinator::Toggle(
  ...
  LOG(INFO) << "Toggle: Entry at start, IsSidePanelShowing=" << IsSidePanelShowing()
            << ", IsSidePanelEntryShowing=" << IsSidePanelEntryShowing(key);
  ...
  if (IsSidePanelEntryShowing(key) &&
    ...
    LOG(INFO) << "Toggle: Taking path 1 - entry already showing, closing";
    Close();
    return;
  }
  ...
  if (IsSidePanelShowing()) {
    LOG(INFO) << "Toggle: Side panel is showing, checking loading entry";
    if (waiter_->loading_entry() == GetEntryForKey(key)) {
      LOG(INFO) << "Toggle: Taking path 2 - loading entry matches, closing";
      waiter_->ResetLoadingEntryIfNecessary();
      Close();
      return;
    }
    LOG(INFO) << "Toggle: Loading entry doesn't match, continuing";
  }

  std::optional<UniqueKey> unique_key = GetUniqueKeyForKey(key);
  if (unique_key.has_value()) {
    LOG(INFO) << "Toggle: Calling Show() with unique_key";
    Show(unique_key.value(), open_trigger, /*suppress_animations=*/false);
  } else {
    LOG(INFO) << "Toggle: unique_key is nullopt, NOT calling Show()";
  }
}
...
void SidePanelCoordinator::Show(
  ...
  LOG(INFO) << "Show: Called with entry_key=" << input.key.ToString()
  ...
  if (!browser_view_->browser()->is_type_normal()) {
    LOG(INFO) << "Show: Early return - not a normal browser";
    return;
  }

  SidePanelEntry* entry = GetEntryForUniqueKey(input);
  LOG(INFO) << "Show: Got entry, pointer=" << (entry ? "NON-NULL" : "NULL");
...
SidePanelCoordinator::GetUniqueKeyForKey(
    const SidePanelEntry::Key& entry_key) const {
  LOG(INFO) << "GetUniqueKeyForKey called for: " << entry_key.ToString();

  if (GetActiveContextualRegistry() &&
      GetActiveContextualRegistry()->GetEntryForKey(entry_key)) {
    LOG(INFO) << "GetUniqueKeyForKey: Found in contextual registry";
    return UniqueKey{
        browser_view_->browser()->GetActiveTabInterface()->GetHandle(),
        entry_key};
  }

  SidePanelEntry* entry = window_registry_->GetEntryForKey(entry_key);
  LOG(INFO) << "GetUniqueKeyForKey: window_registry_->GetEntryForKey returned: " << (entry ? "NON-NULL" : "NULL");
  if (entry) {
    LOG(INFO) << "GetUniqueKeyForKey: Found in window registry, returning UniqueKey";
    return UniqueKey{/*tab_handle=*/std::nullopt, entry_key};
  }
  LOG(INFO) << "GetUniqueKeyForKey: NOT FOUND, returning nullopt";
  return std::nullopt;
}
...
void SidePanelCoordinator::PopulateSidePanel(
    ...
    SidePanelEntry* entry,
    std::optional<std::unique_ptr<views::View>> content_view) {
  LOG(INFO) << "PopulateSidePanel: Called for entry_key=" << unique_key.key.ToString();
  LOG(INFO) << "PopulateSidePanel: suppress_animations=" << suppress_animations;
  ...
  LOG(INFO) << "PopulateSidePanel: Updated panel icon and title";
  ...
  LOG(INFO) << "PopulateSidePanel: Got content_wrapper, children count=" << content_wrapper->children().size();

  content_wrapper->SetVisible(true);
  LOG(INFO) << "PopulateSidePanel: Set content_wrapper visible, about to call unified_side_panel()->Open()";
  browser_view_->unified_side_panel()->Open(/*animated=*/!suppress_animations);
  LOG(INFO) << "PopulateSidePanel: unified_side_panel()->Open() completed";
