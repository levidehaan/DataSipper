...
void ChromeBrowserMainParts::PreCreateMainMessageLoop() {
  ...
  // DataSipper startup notification
  LOG(INFO) << "🔵 DataSipper: Browser starting up with DataSipper features";
  LOG(INFO) << "🔵 DataSipper: kDataSipperEnabled = "
            << base::FeatureList::IsEnabled(features::kDataSipperEnabled);
  LOG(INFO) << "🔵 DataSipper: kDataSipperNetworkInterception = "
...
void ChromeBrowserMainParts::PostCreateMainMessageLoop() {
  ...
  // Note: DataSipperService is now created per-profile via DataSipperServiceFactory
  // Mojo network client will be initialized in PostBrowserStart()
...
void ChromeBrowserMainParts::PostBrowserStart() {
  ...
  // Initialize DataSipper Mojo network client if network interception is enabled
  // This must be done after the network service is fully initialized
  if (base::FeatureList::IsEnabled(features::kDataSipperEnabled) &&
      base::FeatureList::IsEnabled(features::kDataSipperNetworkInterception)) {
    // Create the browser-side Mojo receiver
    auto network_client = std::make_unique<DataSipperNetworkClient>();

    // Get a PendingRemote from the receiver
    mojo::PendingRemote<datasipper::mojom::DataSipperNetworkClient> remote =
        network_client->BindNewPipeAndPassRemote();

    // Pass the remote to the network service
    content::GetNetworkService()->SetDataSipperClient(std::move(remote));

    LOG(INFO) << "✅ DataSipper: Mojo network client initialized and sent to network service";

    // Store the client to keep it alive
    datasipper_network_client_ = std::move(network_client);
  }
}
