...
void WebSocketChannel::SendAddChannelRequest(
  ...
  // Initialize DataSipper WebSocket observer
  datasipper_observer_ = std::make_unique<DataSipperWebSocketObserver>(socket_url);
...
WebSocketChannel::ChannelState WebSocketChannel::SendFrame(
  ...
  // Notify DataSipper observer about sent frame
  if (datasipper_observer_ && buffer) {
    auto span = buffer->first(buffer_size);
    std::vector<char> data;
    data.reserve(span.size());
    for (uint8_t byte : span) {
      data.push_back(static_cast<char>(byte));
    }
    datasipper_observer_->OnSendFrame(fin, op_code, data);
  }
...
void WebSocketChannel::OnConnectSuccess(
  ...
  if (datasipper_observer_ && response) {
...
ChannelState WebSocketChannel::HandleFrame(
  ...
  // Notify DataSipper observer about received frame
  if (datasipper_observer_) {
    base::span<const char> payload_span = base::as_chars(frame->payload);
    std::vector<char> data(payload_span.begin(), payload_span.end());
    datasipper_observer_->OnReceiveFrame(frame->header.final, opcode, data);
  }
